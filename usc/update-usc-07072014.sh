...
# TODO
# O seguinte comando substitue a imaxe que aparece na xanela de inicio de sesión
# habería que modificar o ficheiro correspondente.
...
sed -i -r -e 's/minino-wallpaper-1_1280x1024.jpg/wallpaper-usc.png/g' /etc/skel/.config/pcmanfm/LXDE/pcmanfm.conf
...
# Modificamos o navegador por defecto que aparece no panel inferior, no lugar de midori
# poñemos o iceweasel

sed -i -r -e 's/\/usr\/bin\/x-www-browser %u/\/usr\/bin\/iceweasel/g' /usr/share/applications/lxde-x-www-browser.desktop
sed -i -r -e 's/\/usr\/bin\/x-www-browser %u/\/usr\/bin\/iceweasel/g' /usr/local/share/applications/lxde-x-www-browser.desktop

sed -i -r -e 's/midori/iceweasel/g' /usr/share/applications/lxde-x-www-browser.desktop
sed -i -r -e 's/midori/iceweasel/g' /usr/local/share/applications/lxde-x-www-browser.desktop

# Desinstalamos o aplicativo macchanger

apt-get remove -y macchanger

# Eliminamos a súa entrada do menú principal

rm /usr/local/share/applications/minino/mac-spoof.desktop

echo "Traballo rematado. Desfrute do seu minino !!"
